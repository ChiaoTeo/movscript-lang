...
import { resolveMovScriptCandidateSelection } from '@movscript/decision'
...
export function prepareContentProductionContext(
  ...
  const settings = queryMovScriptWorkspaceEntities(index, { entityKind: 'setting' })
    .filter((entity) => entity.id !== undefined && [...settingIds].some((id) => sameEntityRef(entity.id, id, 'setting')))
  const settingStates = queryMovScriptWorkspaceEntities(index, { entityKind: 'setting_state' })
    .filter((entity) => entity.id !== undefined && [...settingStateIds].some((id) => sameEntityRef(entity.id, id, 'setting_state')))
  ...
  const sceneKeyframes = sceneMomentDir
    ...
    : []
  const contentUnitDir = contentUnit.path.replace(/\/content_unit\.json$/, '')
  const contentUnitKeyframes = queryMovScriptWorkspaceEntities(index, { entityKind: 'keyframe' })
    .filter((entity) => entity.path.startsWith(`${contentUnitDir}/keyframes/`) || sameId(entity.record.content_unit_id, contentUnit.id))
...
function referenceResourceForEntity(
  ...
  const decision = resolveMovScriptCandidateSelection(entity.record)
  if (!decision) return undefined
  const candidateId = decision.candidateId
  const resourceId = decision.resourceId
