...
export function movScriptWorkspaceAssetPath(input: { id: string; settingId?: string; settingStateId?: string }): string {
  ...
  if (input.settingStateId) return `settings/${settingSlug}/states/${entityPathSlug(input.settingStateId, 'setting_state')}/assets/${assetSlug}/asset.json`
  return `settings/${settingSlug}/assets/${assetSlug}/asset.json`
}
